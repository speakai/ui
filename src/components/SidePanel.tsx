...
export interface SidePanelProps extends HTMLAttributes<HTMLDivElement> {
  ...
  footer?: ReactNode;
  /**
   * Stable identifier for persisting the dragged width in localStorage
   * (`sidepanel-width:<id>`). Falls back to `title` when omitted.
   */
  panelId?: string;
  /**
   ...
   * Desktop only — the panel stays full-width on mobile.
   */
  resizable?: boolean;
...
export const SidePanel = forwardRef<HTMLDivElement, SidePanelProps>(
  (
    ...
    // A width persisted on a large screen could overflow a smaller one — clamp to
    // the viewport whenever the panel opens (and on window resize while open).
    useEffect(() => {
      if (!resizable || !open || typeof window === "undefined") return;
      const clampToViewport = () => {
        const cap = Math.min(maxWidth, window.innerWidth - 80);
        if (widthRef.current > cap) {
          widthRef.current = Math.max(minWidth, cap);
    ...
    }, [resizable, open, minWidth, maxWidth]);
      ...
      <>
        ...
        <div
          ...
          className={cn(
            ...
            resizable ? "w-full sm:w-[var(--sp-w)]" : sizeMap[size],
            className
          )}
          {...props}
          style={
